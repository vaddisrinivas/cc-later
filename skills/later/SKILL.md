...
Each `##` section becomes **one parallel background agent**. When cc-later dispatches, all sections run simultaneously. Each agent operates in its own isolated git worktree on a dedicated branch, so agents cannot conflict on files. When agents finish, their branches are merged back.
...
- **Sections do not overlap on files** — when `DISPATCH_ALLOW_FILE_WRITES=true`, each section runs on its own git branch; two sections editing the same file will cause a merge conflict on reconcile
